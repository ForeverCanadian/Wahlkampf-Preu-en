/* =====================================================================
   Voter flow (Wählerwanderung) 1921 -> 1924
   ---------------------------------------------------------------------
   Every party has two voter-behaviour parameters:
     loyalty     % of a party's surviving 1921 voters who vote for it again
   ...
   The flow matrix is always consistent with the 1921 result and with the
   simulated 1924 result: retained voters follow loyalty, everything else
   ...
   Units: percent of the 1921 electorate (= 100).
   ===================================================================== */
...
function voterFlowModel(v0in,s1in,swings,voter,cfg){
  ...
  for(let i=0;i<n;i++){
    ...
    A.push(clamp(FLOW_K.abstain*(2-E[i]/50),0,.6));           // abstention among 1921 voters
  }
  ...
  // marginals (percent of the 1921 electorate)
  const R=v0.map(x=>x*T0).concat([100*(1-T0),F]);                 // parties, non-voters 1921, first-time voters
  const elec1=100-D+F,votes1=T1*elec1;
  const C=s1.map(x=>x/100*votes1).concat([(1-T1)*elec1,D]);       // parties, non-voters 1924, departed
  // stage 1: retained voters (loyalty), capped by what the party can still hold in 1924
...
<text x="18" y="16" class="subtitle">Landtagswahl in Preußen 1924</text><text x="18" y="34" class="title">Wählerwanderung</text>
<text x="${xL+nw/2}" y="62" text-anchor="middle" class="fyear">1921</text><text x="${xR+nw/2}" y="62" text-anchor="middle" class="fyear">1924</text>`;
...
<text x="304" y="512" text-anchor="middle" class="credit">Bandbreite = Anteil der Wahlberechtigten · Bewegen Sie die Maus über Bänder und Balken</text>
...
function flowTipLines(vf,el){
  const n=vf.n,M=vf.M,R=vf.R,C=vf.C,de=flowDe;
  const namesL=vf.names.concat(["Nichtwähler 1921","Erstwähler"]),namesR=vf.names.concat(["Nichtwähler 1924","Ausgeschieden"]);
  ...
  if(side==="L"){
    if(k<n){let off=0;for(let j=0;j<n;j++)if(j!==k)off+=M[k][j];
      return[`${namesL[k]} 1921`,`Stimmenanteil ${de(vf.v0[k])} % · hielten die Treue: ${pct(M[k][k],R[k])}`,`wechselten zu anderen Parteien: ${pct(off,R[k])}`,`Nichtwähler: ${pct(M[k][n],R[k])} · ausgeschieden: ${pct(M[k][n+1],R[k])}`];}
    if(k===n)return[`Nichtwähler 1921 (${de(R[n])} % der Wahlberechtigten)`,`blieben zu Hause: ${pct(M[n][n],R[n])}`,`gingen 1924 wählen: ${pct(sumRow(n,0,n),R[n])}`,`ausgeschieden: ${pct(M[n][n+1],R[n])}`];
  ...
  if(k<n){let oth=0;for(let i=0;i<n;i++)if(i!==k)oth+=M[i][k];const dlt=vf.s1[k]-vf.v0[k];
    return[`${namesR[k]} 1924`,`Stimmenanteil ${de(vf.s1[k])} % (${dlt>=0?"+":""}${de(dlt)})`,`Stammwähler von 1921: ${pct(M[k][k],C[k])} · andere Parteien: ${pct(oth,C[k])}`,`frühere Nichtwähler: ${pct(M[n][k],C[k])} · Erstwähler: ${pct(M[n+1][k],C[k])}`];}
  if(k===n)return[`Nichtwähler 1924 (${de((1-vf.T1)*100)} % der Wahlberechtigten)`,`frühere Nichtwähler: ${pct(M[n][n],C[n])}`,`ehemalige Parteiwähler: ${pct(sumCol(n,0,n),C[n])}`,`Erstwähler: ${pct(M[n+1][n],C[n])}`];
  return[`Ausgeschieden (gestorben / fortgezogen)`,`${de(C[n+1])} % der Wahlberechtigten von 1921`,`aus Parteien: ${pct(sumCol(n+1,0,n),C[n+1])} · aus Nichtwählern: ${pct(M[n][n+1],C[n+1])}`];
}
...
function drawTurnoutGraph(svg,vf){
  ...
  const segC=vf.names.map((nm,i)=>({label:nm,value:M[i][n],color:vf.colors[i]})).concat([{label:"schon 1921 Nichtwähler",value:M[n][n],color:FLOW_NW},{label:"Erstwähler",value:M[n+1][n],color:FLOW_FT}]);
...
<text x="18" y="16" class="subtitle">Landtagswahl in Preußen 1924</text><text x="18" y="34" class="title">Erstwähler und Nichtwähler</text>
<text x="${x0}" y="62" class="hd">Wahlbeteiligung</text>
<text x="${x0}" y="81" class="tl">1921</text><rect x="${tx}" y="70" width="${tw}" height="14" fill="#eceef0"/><rect x="${tx}" y="70" width="${(tw*vf.T0).toFixed(1)}" height="14" fill="#6b7280"/>
<text x="${(tx+tw*vf.T0-6).toFixed(1)}" y="81" text-anchor="end" class="segnum" style="fill:#fff">${de(vf.T0*100)} %</text>
<text x="${x0}" y="101" class="tl">1924</text><rect x="${tx}" y="90" width="${tw}" height="14" fill="#eceef0"/><rect x="${tx}" y="90" width="${(tw*vf.T1).toFixed(1)}" height="14" fill="#374151"/>
...
<text x="${x0}" y="202" class="hd">Nichtwähler von 1921 (${de(R[n])} %) – wie sie 1924 handelten</text>${stack(segB,x0,209,bw,26)}
<text x="${x0}" y="266" class="hd">Nichtwähler 1924 (${de((1-vf.T1)*100)} %) – woher sie kamen</text>${stack(segC,x0,273,bw,26)}`;
  items.forEach((it,k)=>{
...
function renderFlowControls(){
  ...
  box.dataset.ready="1";
  const defs=[["turnout0","1921 turnout (%)",40,95,.5],["firstTimers","First-time voters (% of electorate)",0,15,.1],["departures","Leaving the electorate (%)",0,15,.1]];
  ...
  if(note)note.textContent="Estimated flows. Voter loyalty sets how many of a party's surviving 1921 voters stay; the rest are spread by ideological proximity and fitted to the 1921 result and the simulated 1924 result. Enthusiasm sets abstention and overall turnout. The economic polling swings shift both (loyalty \u00b11.2, enthusiasm \u00b11.5 per swing point). These parameters do not change the seat result yet.";
}